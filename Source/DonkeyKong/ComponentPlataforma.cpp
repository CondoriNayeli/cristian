...
void AComponentPlataforma::Tick(float DeltaTime)
{
	...
	if (moverPlataformaMedio)  //Debemos mover la plataforma 
	{
		FVector posicionActual = GetActorLocation();  //Debemos mover la plataforma?
		FVector nuevaPosicion;

		if (moviendoHaciaObjetivo) //stamos moviendo la plataform
		{
			...
			{
				moviendoHaciaObjetivo = false;         //Muévete desde donde estás ahora (posicionActual) hacia donde quieres ir (posicionObjetivo), a una velocidad fija (velocidadMovimient
			}
		}
		else {
